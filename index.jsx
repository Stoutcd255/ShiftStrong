...
import MobileFitnessScreens from './src/components/MobileFitnessScreens';
...
function App() {
  ...
  const macroAdherence = goals.calories ? Math.min(100, Math.round((todayTotals.calories / goals.calories) * 100)) : 0;
  const getSignal = (value, high = 80, mid = 60) => (value >= high ? 'green' : value >= mid ? 'yellow' : 'red');
  const readinessSignal = getSignal(readinessScore, 85, 70);
  const macroSignal = getSignal(Math.abs(100 - macroAdherence) <= 15 ? 100 : 55, 80, 60);
  const volumeSignal = getSignal(workoutVolumeWeek >= 6000 ? 90 : workoutVolumeWeek >= 3000 ? 65 : 45, 80, 60);
      ...
      <nav className="tab-row" aria-label="Main sections">
        {tabs.map((tab, index) => (
          <button key={tab.id} type="button" className={activeTab === tab.id ? 'tab active' : 'tab'} onClick={() => setActiveTab(tab.id)}>
            {tab.label} <span className="shortcut">Ctrl+{index + 1}</span>
          </button>
        ))}
      </nav>

      <section className="quick-action-dock" aria-label="Quick actions">
        ...
        {activeTab === 'home' && (
          <section className="grid two-col">
            <article className="card spotlight">
              <h2>Command Center Snapshot</h2>
              <div className="signal-row">
                <span className={`signal-pill ${readinessSignal}`}>Readiness</span>
                <span className={`signal-pill ${macroSignal}`}>Intake</span>
                <span className={`signal-pill ${volumeSignal}`}>Training Load</span>
              </div>
              <ul className="quick-list">
                <li><strong>Latest Weight:</strong> {latestWeight} lbs</li>
                <li><strong>Workout Blocks:</strong> {workouts.length}</li>
                <li><strong>Weekly Planned Minutes:</strong> {weeklyDuration}</li>
                <li><strong>7-Day Training Volume:</strong> {Math.round(workoutVolumeWeek)} lb</li>
                <li><strong>Readiness Score:</strong> {readinessScore}</li>
                <li><strong>Shift State:</strong> {dutyMode === 'on-duty' ? 'On-Duty (Rapid Logging)' : 'Off-Duty (Recovery Planning)'}</li>
              </ul>
              <p className="guidance">{intensityGuidance}</p>
            </article>

            <article className="card">
              <h2>Rank Progression ({FALLBACK_DATE()})</h2>
              <ul className="quick-list">
                <li><strong>Current Rank:</strong> {currentRank}</li>
                <li><strong>Next Rank:</strong> {nextRank}</li>
                <li><strong>Points to Promotion:</strong> {pointsToNextRank}</li>
                <li><strong>Total Points:</strong> {progressionPoints}</li>
              </ul>
            </article>

            <article className="card">
              <h2>Performance Progress ({FALLBACK_DATE()})</h2>
              <ul className="quick-list">
                <li><strong>Protein:</strong> {todayTotals.protein}/{goals.protein}g ({goalsProgress.proteinPct}%)</li>
                <li><strong>Carbs:</strong> {todayTotals.carbs}/{goals.carbs}g ({goalsProgress.carbsPct}%)</li>
                <li><strong>Fats:</strong> {todayTotals.fats}/{goals.fats}g ({goalsProgress.fatsPct}%)</li>
                <li><strong>Calories:</strong> {todayTotals.calories}/{goals.calories} ({goalsProgress.caloriesPct}%)</li>
                <li><strong>Sessions:</strong> {workouts.length}/{goals.sessionsPerWeek} ({goalsProgress.sessionPct}%)</li>
              </ul>
            </article>

            <article className="card full-width">
              <h2>Periodization + Auto Programming</h2>
              <div className="split-grid">
                <form className="form-grid three" onSubmit={(e) => e.preventDefault()}>
                  <label>Phase
                    <select value={periodization.phase} onChange={(e) => setPeriodization((prev) => ({ ...prev, phase: e.target.value }))}>
                  ...
                  <label>Week
                    <input type="number" min="1" max="12" value={periodization.week} onChange={(e) => setPeriodization((prev) => ({ ...prev, week: Math.max(1, Math.min(12, Math.round(toNumber(e.target.value)))) }))} />
                  </label>
                  <label>Total Weeks
                    <input type="number" min="1" max="12" value={periodization.totalWeeks} onChange={(e) => setPeriodization((prev) => ({ ...prev, totalWeeks: Math.max(1, Math.min(12, Math.round(toNumber(e.target.value)))) }))} />
                  </label>
                </form>
                <div className="inline-actions">
                  <button type="button" className="btn" onClick={autoProgramNextWeek}>Generate Next Week Plan</button>
                </div>
              </div>
            </article>

            <article className="card full-width">
              <h2>Progression Assistant</h2>
              <ul className="data-list compact">
                {recommendedLoadList.length === 0 && <li>No exercise history yet. Log workouts to receive load suggestions.</li>}
                {recommendedLoadList.map((item) => (
                  <li key={item.name}>
                    <span>{item.name}</span>
                    <strong>Avg {item.avgLoad} lb @ RPE {item.avgRpe} → Next {item.nextLoad} lb</strong>
                  </li>
                ))}
              </ul>
            </article>
