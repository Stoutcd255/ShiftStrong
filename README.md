...
## If GitHub rejects pushes because of image/binary files
This project uses image assets for the tactical background/splash.  
To keep those images while avoiding binary-push issues, use **Git LFS**:

1. Install Git LFS: https://git-lfs.com  
2. Run:
   ```powershell
   git lfs install
   git lfs track "assets/*.png" "assets/*.jpg" "assets/*.jpeg" "assets/*.webp"
   git add .gitattributes
   ```
3. If images were already committed as normal Git objects, migrate them to LFS:
   ```powershell
   git lfs migrate import --include="assets/*.png,assets/*.jpg,assets/*.jpeg,assets/*.webp"
   ```
4. Push again:
   ```powershell
...
This keeps your splash/background images in the app, but stores them via LFS pointers so GitHub push policies are easier to satisfy.
