...
export const selectTrendRange = (trendData, rangeDays = 30) => {
  const safeRange = Math.max(1, Math.min(365, Math.round(toNumber(rangeDays) || 30)));
  return (Array.isArray(trendData) ? trendData : []).slice(-safeRange);
};

export const buildRecentActivity = ({ weightLogs = [], macroLogs = [], workouts = [], exerciseLogs = [], fallbackDate = FALLBACK_DATE() }) => {
  const weightActivity = weightLogs.map((entry) => ({ date: entry.date, label: 'Weight Check-In', detail: `${entry.weight} lb` }));
  const macroActivity = macroLogs.map((entry) => ({ date: entry.date, label: 'Macro Log', detail: `${entry.calories} cal` }));
  const workoutActivity = workouts.map((entry) => ({ date: fallbackDate, label: 'Program Block', detail: `${entry.day} · ${entry.duration} min` }));
  const exerciseActivity = exerciseLogs.map((entry) => ({ date: entry.date, label: entry.name, detail: `${entry.sets}x${entry.reps} @ ${entry.load} lb` }));

  return [...weightActivity, ...macroActivity, ...workoutActivity, ...exerciseActivity]
